...
function [pval, t_orig, clust_info, seed_state, est_alpha, mn_clust_mass]=clust_perm1_iceeg_count_rand(data,chan_hood,n_perm,fwer,tail,thresh_p,verblevel,seed_state,freq_domain)
...
sn=zeros(1,1,n_subs);
sn_test=zeros(n_chan,n_pts,n_subs);
for perm=1:n_perm
    if ~rem(perm,100)
        if (verblevel>=2),
...
%     sn_mtrx=repmat(sn,[n_chan n_pts 1]);
    ...
    % test (random)
    sn_test(1:n_chan,1:n_pts,1:n_subs)=(rand(n_chan,n_pts,n_subs)>.5)*2-1;
    sn_mtrx=sn_test;
