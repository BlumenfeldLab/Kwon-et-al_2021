...
prefix='Wendy_hunki_std_5_20_base';

% folder_name=['Power_plot_std_5_20_base'];
% mkdir(folder_name)
...
%             fdata_new_trial=fpower(:,k);
%             for window_i=1:39
%                 samples_in_window=fdata_new_trial(samples(window_i):samples(window_i)+window_size-1,1);
%                 samples_in_window(find(zscore(samples_in_window)>2),1)=NaN;
%                 trialpowers_window(window_i,k)=nanmean(samples_in_window);
%             end
%         end
...
%         set(gca,'fontsize',12);
        ...
        %% test
%         base_vals=trialpowers_window_buffer(1:9,:);
%         figure;
%         histogram(base_vals(:))
        
        mean_base_vals=nanmean(trialpowers_window_buffer(1:9,:),1);
%         figure;
%         histogram(mean_base_vals(:))
        
        trialpowers_window_buffer(:,(zscore(mean_base_vals)<-2 | zscore(mean_base_vals)>2))=[];
%         plot(trialpowers_window_buffer(:,(zscore(mean_base_vals)<-2 | zscore(mean_base_vals)>2)))

%         var_base_vals=var(trialpowers_window_buffer(1:9,:));
%         histogram(var_base_vals(:))
