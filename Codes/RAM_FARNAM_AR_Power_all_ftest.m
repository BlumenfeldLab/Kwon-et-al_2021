...
function RAM_FARNAM_AR_Power_all_ftest(i,subject_name)
...
%     end
% end
% save(['rejection_idx_' prefix '.mat'],'noisy_RAM_all_trials_idx',...
...
ranksum_p_traces=NaN(size(location_data_pair,2),1);
ftest_p_traces=NaN(size(location_data_pair,2),1);
noisy_RAM_all_trials_power_ratio=nan(node_n,1);

% folder_name='Power_plot';
% mkdir(folder_name)
for node_i=1:node_n
    try
        trialpowers_window=[];
        ...
        if noisy_RAM_all_trials_number(node_i)/RAM_all_trials_total_number < 0.3
            fpower = [];
            trialpowers_window = [];
    ...
    %         %% plot power of all trials (RAW)
    %         mean_trialpowers_window=[];
    %         mean_trialpowers_window = squeeze(mean(trialpowers_window,2));
    %         base_std=std(mean_trialpowers_window(1:base_sample_idx));
    %         base_mean=mean(mean_trialpowers_window(1:base_sample_idx));
    %         z_mean_trialpowers_window_raw=[];
    %         z_mean_trialpowers_window_raw=(mean_trialpowers_window-base_mean)/base_std;
    %         
    ...
    %         xlabel('Time (s)','Fontsize',12);
    %         ylabel('Average Gamma power','Fontsize',12);
    %         ylim([0 10])
    %         set(gca,'fontsize',12);
    %         
    %         subplot(3,2,5)
    %         plot(TIME,z_mean_trialpowers_window_raw);
    %         ylabel('Gamma Power z-score','Fontsize',12);
    %         ylim([-6,8]);
    %         set(gca,'fontsize',12);
    %         line([-0.5,1.5],[2,2],'color','r');
    %         line([-0.5,1.5],[-2,-2],'color','r');

            mean_power= nanmean(trialpowers_window(:));
            std_power = nanstd(trialpowers_window(:));

            std_num =5;
            trialpowers_window(trialpowers_window > mean_power + std_num*std_power | trialpowers_window < mean_power - std_num*std_power) = NaN;

            noisy_trials_power_idx=[];
            for h = 1:size(trialpowers_window,2)
                if ~isempty(find(isnan(trialpowers_window(:,h)),1))       
                    noisy_trials_power_idx = [noisy_trials_power_idx h];       
                end
            end

            noisy_RAM_all_trials_power_ratio(node_i,1) = length(noisy_trials_power_idx)/size(trialpowers_window,2);
            noisy_RAM_all_trials_power_idx{node_i}=noisy_trials_power_idx;
            ...
            % stats
            if ~isnan(z_mean_trialpowers_window)
                mean_trialpowers_window=abs(z_mean_trialpowers_window);
                ranksum_p_traces(node_i,1)=ranksum(mean_trialpowers_window(1:base_sample_idx),mean_trialpowers_window(base_sample_idx+1:end));
                mean_trialpowers_window=z_mean_trialpowers_window;
                [h,p,ci,stats]=vartest2(mean_trialpowers_window(1:base_sample_idx),mean_trialpowers_window(base_sample_idx+1:end));
                ftest_p_traces(node_i,1)=p;
            end
    %         %% plot power of all trials (CLEAN)
    %         subplot(3,2,2)
    %         plot(TIME,trialpowers_window); hold on;
    %         title(['Artifact rate : ' num2str(noisy_RAM_all_trials_power_ratio(node_i,1)) '%'])
    %         xlabel('Time (s)','Fontsize',12);
    %         ylabel('Gamma power (40-115Hz) ','Fontsize',12);
    %         ylim([0 100])
    %         set(gca,'fontsize',12);
    ...
    %         line([-0.5,1.5],[-2,-2],'color','r');
    % 
    %         saveas(gcf,[folder_name '/plot_power_elec_'  num2str(node_i) '.png'])
    %         close all;
...
save(['rejection_power_idx_' prefix '.mat'],'noisy_RAM_all_trials_power_idx','noisy_RAM_all_trials_power_ratio','-v7.3')

% ranksum_p_traces_rest=ftest_p_traces;
% ranksum_p_traces_rest(Atlas_subject_elec_val)=NaN;
% Atlas_subject_elec_val=[Atlas_subject_elec_val find(ranksum_p_traces_rest<0.05)'];

patient_filename = ['./stats_traces_' prefix '.mat'];
save(patient_filename,'zscore_traces','ranksum_p_traces','Atlas_subject_elec_val','location_data_pair','T','-v7.3')
...
patient_filename = ['./stats_traces_' prefix '_ftest_fdr.mat'];
save(patient_filename,'zscore_traces','ftest_p_traces','Atlas_subject_elec_val_fdr','location_data_pair','T','-v7.3')
...
L_MontageMap=[];
R_MontageMap=[];
Montage_prefix_fdr='final_clean_dil_Wendy_ftest_fdr';
