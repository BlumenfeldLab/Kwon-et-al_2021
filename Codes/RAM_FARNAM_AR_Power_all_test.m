...
%         all_trials_node=squeeze(RAM_all_trials(node_i,:,:));
%         
...
%         if noisy_RAM_all_trials_number(node_i)/RAM_all_trials_total_number < 0.3
    ...
    %         line([-0.5,1.5],[-2,-2],'color','r');
            ...
            mean_trialpowers_window=[];
            mean_trialpowers_window = squeeze(mean(trialpowers_window,2));
            base_std=std(mean_trialpowers_window(3:base_sample_idx));
            base_mean=mean(mean_trialpowers_window(3:base_sample_idx));
            ...
            if ~isnan(z_mean_trialpowers_window)
                ...
                ranksum_p_traces(node_i,1)=ranksum(mean_trialpowers_window(3:base_sample_idx),mean_trialpowers_window(base_sample_idx+1:end));
%                 ttest2(mean_trialpowers_window(1:base_sample_idx),mean_trialpowers_window(base_sample_idx+1:end));
            end
...
% load(['rejection_power_idx_' prefix '_merge.mat'])

% ranksum_p_traces_rest=ranksum_p_traces;
