...
function displayElectrodesInflated_new_fs_Atlas_n(data_location, overlay, laterality, views,electrodeDensity, thr,atlas_suffix,roi_neighbor_matrix,atlas_index)

mni_nifti_path = 'MNI_T1_1mm_stripped.nii';
mnit1_1mm_stripped = mni2fs_load_nii(mni_nifti_path);
Tmni = mnit1_1mm_stripped.transform;
alpha = 0.7;
colors = distinguishable_colors(size(roi_neighbor_matrix,1)); % generate different colors
...
%             atlas_cmap(atlas_index,:)=[0 0 0];
            colors(atlas_index,:)=[0 0 0];

            atlas_name=colortable.struct_names{atlas_index};

            image_storage_folder = [data_location '/Storage_filtering_overlap_base/Atlas_' atlas_suffix '/' num2str(atlas_index)];
        ...
        for frame_index = 1
            ...
            if side_index == 1
                roi_neighbor_index=find(roi_neighbor_matrix(atlas_index,:)>0);
                fs_roi_index_neighbor=colortable.table(roi_neighbor_index,5);
                for neighbor_i=1:size(roi_neighbor_index,2)
                    vertex_values_frame(find(label==fs_roi_index_neighbor(neighbor_i)))=roi_neighbor_index(neighbor_i);
                end
            end
