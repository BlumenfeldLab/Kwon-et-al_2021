...
function displayElectrodesInflated_new_fs_electrode(data_location, patients, overlay, figure_type, views, electrodeDensity, electrodeDisplay, ...
    thr,file_suffix,storage_suffix,Montage_suffix,all_sub_index)

image_storage_folder = [data_location '/Storage_filtering_overlap_' storage_suffix '/Image_storage/Electrode_display'];
...
alpha = 0.7;
colodrs = distinguishable_colors(length(patients)); % generate different colors
...
%                 colorElectrode = [1 0 0];
                if isempty(electrode) % Some files only have electrodes on the one side
                    ...
                    [elecVertices elecVertices_dis] = ProjectElectrode2TransSurf(v_num, fs_surf(side_index).vertices, all_electrodes, electrode);
                    ...
                    if electrodeDisplay == 1
                        scatter3(inflated_surf(side_index).vertices(elecVertices,1),...
                            ...
                            inflated_surf(side_index).vertices(elecVertices,3), 80, colorElectrode,'filled');
    ...
    if ~ismember(overlay,1)
        for frame_index = 1:total_num_frames
            ...
            if (overlay == 2 || overlay == 3 || overlay == 4) || ~electrodeDensity% the surface is colored according to zscores of power
                if ~electrodeDensity & electrodeDisplay
                    vertex_values_sum = zeros(size(vertex_values_frame,1),1);
                else
...
%         frame(frame_index,:,:,:) = combined_views;
        ...
        if ismember(overlay,[2 3 4])
            if views == 4
                load(xtick_spacing_filename)
                if electrodeDisplay == 0 & electrodeDensity == 0
