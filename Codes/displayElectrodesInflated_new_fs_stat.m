...
function displayElectrodesInflated_new_fs_stat(data_location,patients,overlay, laterality, views, thr,color_range,colormap_mat,file_suffix,storage_suffix,Montage_suffix,atlas_name,atlas_folder,atlas_index,flip_prefix)
...
image_storage_folder = [data_location '/Storage_filtering_overlap_' storage_suffix '/Image_storage/' file_suffix '/' Montage_suffix '/' atlas_index '_tval'];
...
%         load('cmap_hunki_10to15.mat'); 
        ...
        if side_index == 1
            tval_raw=t_orig(1:size(colortable.table,1),:);
        else
            tval_raw=t_orig(size(colortable.table,1)+1:end,:);
        end
