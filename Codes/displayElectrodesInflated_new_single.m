...
function displayElectrodesInflated_new_single(data_location,subject_i, single_patient, overlay, laterality, views, inflationstep ,electrodeDensity, electrodeDisplay, thr,mean_flag,color_range,file_suffix,Montage_suffix,statistic_flag)


image_storage_folder = [data_location '/Storage_filtering_no_overlap/Image_storage/' file_suffix '/' Montage_suffix '/Individual_subject/' num2str(subject_i) '_' single_patient ];
...
alpha = 0.7;
colors = distinguishable_colors(subject_i); % generate different colors
...
%         total_num_frames = 121;
    ...
    % for each patient
    patient_folder = [data_location '/' num2str(p) '_' single_patient];
...
%             load([side '_vertex_values_sum_5_subjects_stats_roi.mat']);
        ...
        for frame_index = 1:total_num_frames
            ...
            vertex_values_frame(:,1)=vertex_values(:,frame_index);
