...
prefix='Wendy_hunki_std_5_20_fs_session';
...
for session_i=1:session_n
    try
        ...
        RAM_all_trials_sessions=cat(3,RAM_Rec_trials_sessions{session_i},RAM_nonRec_trials_sessions{session_i});
        for node_i=1:node_n
            try
                all_trials_node=[];
                all_trials_node=squeeze(RAM_all_trials_sessions(node_i,:,:));
                [~,noisy_idx_buffer]=artifact_rejection_all_together(all_trials_node,fs,0.2);                        
...
save(['rejection_idx_' prefix '.mat'],'noisy_RAM_all_trials_idx','-v7.3')
...
ratio_thr=0.5;
zscore_traces_all=NaN(node_n,1,4,size(T,2));
zscore_traces_Rec=NaN(node_n,1,4,size(T,2));
zscore_traces_nonRec=NaN(node_n,1,4,size(T,2));
noisy_RAM_all_trials_all=NaN(node_n,3);
noisy_RAM_all_trials_Rec=NaN(node_n,3);
noisy_RAM_all_trials_nonRec=NaN(node_n,3);
for node_i=1:node_n
    try
        z_trialpowers_window_all=[];
        z_trialpowers_window_Rec=[];
        z_trialpowers_window_nonRec=[];
        for session_i=1:session_n
            try
                RAM_all_trials_sessions=[];
                RAM_all_trials_sessions=cat(3,RAM_Rec_trials_sessions{session_i},RAM_nonRec_trials_sessions{session_i});
                Rec_n=size(RAM_Rec_trials_sessions{session_i},3);
                nonRec_n=size(RAM_nonRec_trials_sessions{session_i},3);
                Rec_idx=1:Rec_n;
                nonRec_idx=Rec_n+1:Rec_n+nonRec_n;
                
                channel_epochs=squeeze(RAM_all_trials_sessions(node_i,:,:));
                
                % remove bad trials 
                channel_epochs(:,noisy_RAM_all_trials_idx{node_i,session_i})=NaN;
                ...
                base_mean=nanmean(mean(trialpowers_window(1:base_sample_idx,:)));
                base_std=nanstd(mean(trialpowers_window(1:base_sample_idx,:)));
                
                z_trialpowers_window=[];
                z_trialpowers_window=(trialpowers_window-base_mean)/base_std;
                
                z_trialpowers_window_all=[z_trialpowers_window_all z_trialpowers_window];
                z_trialpowers_window_Rec=[z_trialpowers_window_Rec z_trialpowers_window(:,Rec_idx)];
                z_trialpowers_window_nonRec=[z_trialpowers_window_nonRec z_trialpowers_window(:,nonRec_idx)];

            catch
            end
        end
        
        noisy_RAM_all_trials_all(node_i,1)=size(z_trialpowers_window_all,2);
        noisy_RAM_all_trials_all(node_i,2)=length(find(~isnan(z_trialpowers_window_all(1,:))>0));
        noisy_RAM_all_trials_all(node_i,3)=noisy_RAM_all_trials_all(node_i,2)/noisy_RAM_all_trials_all(node_i,1);
        
        noisy_RAM_all_trials_Rec(node_i,1)=size(z_trialpowers_window_Rec,2);
        noisy_RAM_all_trials_Rec(node_i,2)=length(find(~isnan(z_trialpowers_window_Rec(1,:))>0));
        noisy_RAM_all_trials_Rec(node_i,3)=noisy_RAM_all_trials_Rec(node_i,2)/noisy_RAM_all_trials_Rec(node_i,1);
        
        noisy_RAM_all_trials_nonRec(node_i,1)=size(z_trialpowers_window_nonRec,2);
        noisy_RAM_all_trials_nonRec(node_i,2)=length(find(~isnan(z_trialpowers_window_nonRec(1,:))>0));
        noisy_RAM_all_trials_nonRec(node_i,3)=noisy_RAM_all_trials_nonRec(node_i,2)/noisy_RAM_all_trials_nonRec(node_i,1);
        
        if noisy_RAM_all_trials_all(node_i,3)>ratio_thr & noisy_RAM_all_trials_Rec(node_i,3)>ratio_thr & noisy_RAM_all_trials_nonRec(node_i,3)>ratio_thr
            zscore_traces_all(node_i,1,band_i,:)=nanmean(z_trialpowers_window_all,2);
            zscore_traces_Rec(node_i,1,band_i,:)=nanmean(z_trialpowers_window_Rec,2);
            zscore_traces_nonRec(node_i,1,band_i,:)=nanmean(z_trialpowers_window_nonRec,2);
        end
...
% figure;
% plot(squeeze(zscore_traces_Rec(:,1,band_i,:))')
% ylim([-5 10])
% figure;
% plot(squeeze(zscore_traces_nonRec(:,1,band_i,:))')
% ylim([-5 10])
% figure;
% plot(squeeze(zscore_traces_all(:,1,band_i,:))')
% ylim([-5 10])

patient_filename = ['./stats_traces_' prefix '_all.mat'];
save(patient_filename,'zscore_traces','noisy_RAM_all_trials','location_data_pair','T','-v7.3')

zscore_traces=[];
noisy_RAM_all_trials_all=[];
zscore_traces=zscore_traces_Rec;
noisy_RAM_all_trials_all=noisy_RAM_all_trials_Rec;
patient_filename = ['./stats_traces_' prefix '_Rec.mat'];
save(patient_filename,'zscore_traces','noisy_RAM_all_trials','location_data_pair','T','-v7.3')

zscore_traces=[];
noisy_RAM_all_trials_all=[];
zscore_traces=zscore_traces_nonRec;
noisy_RAM_all_trials_all=noisy_RAM_all_trials_nonRec;
patient_filename = ['./stats_traces_' prefix '_nonRec.mat'];
save(patient_filename,'zscore_traces','noisy_RAM_all_trials','location_data_pair','T','-v7.3')
...
%% original
task_name={'Rec','nonRec','all'};
for task_i=1:3
    file_suffix=['Wendy_hunki_std_5_20_fs_session_' task_name{task_i} ];
    Montage_prefix='final_roi_fs';
    displayElectrodesInflated_new_FARNAM(overlay,file_suffix,Montage_prefix) %, frames_set)
end

end

function displayElectrodesInflated_new_FARNAM(overlay,file_suffix,Montage_prefix)
