...
prefix='Wendy_hunki_std_5_20_fs';
...
%     'noisy_RAM_all_trials_number','RAM_all_trials_total_number','-v7.3')
load(['rejection_idx_Wendy_hunki_std_5_20.mat'])


% power
zscore_traces=NaN(size(location_data_pair,2),1,4,size(T,2)-1);
zscore_traces_Kate=NaN(size(location_data_pair,2),1,4,size(T,2)-1);
...
%                 trialpowers_window(:,k)=nanmean(reshape(data_buf(actual_time,:),[fs*window_size,2*1/window_size]),1);
        ...
        for k = 1:size(fpower,2)
            ...
            for window_i=1:40
                trialpowers_window(window_i,k)=nanmean(fdata_new_trial(samples(window_i):samples(window_i+1)-1,1));
            end
        end
...
%         trialpowers_window(:,noisy_trials_power_idx) = [];
        ...
        if (size(trialpowers_window,2)>100 & remaining_ratio>0.2)
            %% z-scoring 1
            ...
            % store zscore
            zscore_traces_Kate(node_i,1,band_i,:)=z_mean_trialpowers_window';
...
% plot(squeeze(zscore_traces(2,1,3,:)))
...
patient_filename = ['./stats_traces_' prefix '_xflip.mat'];
save(patient_filename,'zscore_traces','noisy_RAM_all_trials','location_data_pair','T','-v7.3')

zscore_traces = [];
zscore_traces = zscore_traces_Kate;
patient_filename = ['./stats_traces_' prefix '_Kate.mat'];
save(patient_filename,'zscore_traces','noisy_RAM_all_trials','location_data_pair','T','-v7.3')
patient_filename = ['./stats_traces_' prefix '_Kate_xflip.mat'];
save(patient_filename,'zscore_traces','noisy_RAM_all_trials','location_data_pair','T','-v7.3')
...
Montage_prefix='final_roi_fs_xflip';
