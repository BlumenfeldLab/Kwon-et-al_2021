...
%         figure_type: collect data and draw on: 0 = bilateral, 1 = left only, 2 = right only, 3 = project both left and right electrodes on the left brain
...
function displayElectrodesInflated_new_fs_Atlas(data_location, patients, overlay, figure_type, views, thr)
...
%         [vertices, label, colortable]=read_annotation('lh.Yeo2011_7Networks_N1000.annot');
...
%         [vertices, label, colortable]=read_annotation('rh.Yeo2011_7Networks_N1000.annot');
...
%         load('cmap_hunki_10to15.mat');   
        ...
        atlas_cmap=colortable.table(:,1:3);
        atlas_cmap=atlas_cmap/255;
        atlas_name=colortable.struct_names{atlas_index};
        atlas_vertex_index=colortable.table(atlas_index,5);
        
        image_storage_folder = [data_location '/Storage_filtering_overlap/Atlas/' file_suffix '/' num2str(atlas_index) '_' atlas_name];
        mkdir(image_storage_folder)
        
        %% read RAM data
%         cd(data_storage_folder)
%         ROI_vertex_size=size(find(label == atlas_vertex_index),1);
%         atlas_all_vertex_val=nan(ROI_vertex_size,size(all_sub_index,2),size(T,2));
% %         atlas_all_vertex_val=nan(size(all_sub_index,2),size(T,2));
%         for frame_index = 1:total_num_frames
%             disp(['>> Rendering ' side ' Cortex, Frame ' num2str(frame_index) ', ROI : ' atlas_name ]);  % Output Status
% 
%             load([side '_vertex_values_' file_suffix '_' num2str(frame_index) '.mat']);  
%             vertex_values_frame=vertex_values_frame(:,all_sub_index);
%             vertex_values_frame((vertex_values_frame==0))=NaN;
%             
%             atlas_all_vertex_val(:,:,frame_index)=vertex_values_frame(find(label == atlas_vertex_index),:);
% %             atlas_all_vertex_val(:,frame_index)=nanmean(vertex_values_frame(find(label == atlas_vertex_index),:),1)';
%         end
%         
%         %% save vertex data
%         cd(image_storage_folder)
%         if side_index == 1
%             save([side '_vertex_values.mat'],'atlas_all_vertex_val','-v7.3')
%         else
%             save([side '_vertex_values.mat'],'atlas_all_vertex_val','-v7.3')
%         end
        ...
        cd(image_storage_folder)
        for frame_index = 1
            disp(['>> Rendering ' side ' Cortex, ROI : ' atlas_name ]);  % Output Status
            
            vertex_values_frame=zeros(v_num,1);
            vertex_values_frame(find(label == atlas_vertex_index),1)=atlas_index;
            ...
            if ((overlay == 2 || overlay == 3 || overlay == 4) && ndims(vertex_values_frame) == 2) || ~electrodeDensity% the surface is colored according to zscores of power
                if ~electrodeDensity
                    ...
                    colormap(atlas_cmap)
                    caxis([1 length(atlas_cmap)]); % can modify
...
%                         max([0, size(L_Posterior, 1) - size(L_Ventral, 1)]), 3) * 255];
...
%                     imshow(L_Ventral)
                ...
                L_Medial_Padded_n=[ones(200,size(L_Lateral_Padded,2),3)*255; L_Medial_Padded];
                L_Lateral_Padded_n=[ones(200,size(L_Lateral_Padded,2),3)*255; L_Lateral_Padded];
                ...
                elseif figure_type == 3
                    ...
                    combined_views_left{frame_index} = [L_Ventral_L_Posterior, L_Lateral_Medial_Padded,...
                        255*ones(957, 1, 3)];
                elseif figure_type == 4 | figure_type == 5
                    combined_views_left{frame_index} = L_Lateral_Medial_new;
                end
                ...
                if figure_type == 3 && ismember(overlay,[2 3 4])
                    load(xtick_spacing_filename)
            ...
            elseif side_index == 2
                ...
                R_Lateral_Medial_new = [R_Lateral_Padded ones(size(R_Lateral_Padded,1),40,3)*255 R_Medial_Padded];
...
%                     size(combined_views, 1) - uc.Extent(4) - 20, time_txt, 'Fontsize', 20)
    ...
    elseif figure_type == 4
        ...
        if views == 4
            combined_views = [[combined_views_right{frame_index}; ones(max([0, size(combined_views_left{frame_index}, 1) - ...
            size(combined_views_right{frame_index}, 1)]), size(combined_views_right{frame_index}, 2), 3) * 255]...
            ones(size(combined_views_right{frame_index}, 1),80,3)*255 ...
            [combined_views_left{frame_index}; ones(max([0, size(combined_views_right{frame_index}, 1) - ...
            size(combined_views_left{frame_index}, 1)]), size(combined_views_left{frame_index}, 2), 3) * 255] ...
            ones(size(combined_views_right{frame_index}, 1),400,3)*255];
        end
        imshow(combined_views);
        set(gca,'position',[0 0 1 1],'units','normalized')
        axis tight
%         frame(frame_index,:,:,:) = combined_views;
        ...
        if ismember(overlay,[2 3 4])
            ...
            if views == 4
                ...
                text(size(combined_views, 2) -350,...
                    size(combined_views, 1)/2 , time_txt, 'Fontsize', 24)
            end
        elseif overlay == 1
            T = [100 200 300 400 500 600 700 800 900 1000];
            time_txt = ['Time = ' num2str(round((T(frame_index)-0.5)*1000)) 'ms, ' num2str(round((T(frame_index)+0.05-0.5)*1000)) 'ms '];
    ...
    elseif figure_type == 5
        figure
        if views == 4
            combined_views = [combined_views_left{frame_index}];
        end
        imshow(combined_views);
        text(600,100,['ROI : ' atlas_name],'Fontsize',50)
