...
function displayElectrodesInflated_new_fs_Atlas_all_final(roi_result_folder_map,data_location, overlay, figure_type, views, atlas_index_all,atlas_color)
...
%% Side - which side of the brain to draw
start_side = 1;
sides_num = 2;
...
%         [vertices, label, colortable] = read_annotation('lh.aparc.annot');
        [vertices, label, colortable] = read_annotation('lh.aparc.DKTatlas40.annot');
...
%         [vertices, label, colortable] = read_annotation('rh.aparc.annot');
        [vertices, label, colortable] = read_annotation('rh.aparc.DKTatlas40.annot');
...
%         [vertices, label, colortable]=read_annotation('rh.Schaefer2018_100Parcels_7Networks_order.annot');
    ...
    cd(data_location)
    mkdir(roi_result_folder_map)
    cd(roi_result_folder_map)
    if ~ismember(overlay,1)
        
        atlas_vertex_index_all=colortable.table(:,5);
        
%         if ~isempty(atlas_index_all)
%             atlas_vertex_index_buf=atlas_vertex_index_all;
%             atlas_vertex_index_all=nan(size(atlas_vertex_index_all,1),1);
%             atlas_vertex_index_all(atlas_index_all)=atlas_vertex_index_buf(atlas_index_all);
%         end
        ...
        for frame_index = 1
            ...
            for atlas_set_i=1:size(atlas_index_all,2)           
                atlas_set_all=atlas_index_all{atlas_set_i};
                roi_set_vertex=[];
                for atlas_i=atlas_set_all
                    roi_set_vertex_buf=find(label == atlas_vertex_index_all(atlas_i));
                    roi_set_vertex=[roi_set_vertex ;roi_set_vertex_buf];
                end
                vertex_values_frame(roi_set_vertex,1)=atlas_set_i;
            end
