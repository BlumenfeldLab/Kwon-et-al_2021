...
function displayElectrodesInflated_new_fs_stat_cluster(data_location,patients,overlay, laterality, views, thr,color_range,colormap_mat,file_suffix,storage_suffix,Montage_suffix,atlas_name,atlas_folder,atlas_index,flip_prefix,time_thr,zscore_flag)

if zscore_flag == 1
%     image_storage_folder = [data_location '/Storage_filtering_overlap_' storage_suffix '/Image_storage/' file_suffix '/' Montage_suffix '/' atlas_index '_' atlas_name '_zscore_' num2str(time_thr) 'times'];
else
    r_time=(time_thr+1)*25;
    image_storage_folder = [data_location '/Storage_filtering_overlap_' storage_suffix '/Image_storage/' file_suffix '/' Montage_suffix '/' atlas_index '_' atlas_name '_' num2str(r_time) 'ms'];

end
image_storage_folder_z = [data_location '/Storage_filtering_overlap_' storage_suffix '/Image_storage/' file_suffix '/' Montage_suffix ];
data_storage_folder_stats = [data_location '/Storage_filtering_overlap_' storage_suffix '/Data_storage/' file_suffix '/' Montage_suffix '/Atlas_info'];
...
%         all_frames_num=[9 20:30];
%         all_frames_num=[9 32 44 56 68 80 92];
    end
...
%         load('cmap_hunki_10to15.mat'); 
        ...
        if zscore_flag==1
            cd(image_storage_folder_z)
            if side_index == 1
                load(['L_vertex_values_' file_suffix '_all.mat']);
            else
                load(['R_vertex_values_' file_suffix '_all.mat']);
            end
        end
        ...
        for frame_index = 1:total_num_frames
            ...
            if (overlay == 2 || overlay == 3 || overlay == 4) || ~electrodeDensity% the surface is colored according to zscores of power
                cluster_all_index=find(cluster_based_sig_roi(:,frame_index)~=0);
                if ~isempty(cluster_all_index)
%                         % cluster mapping
%                         for cluster_i=1:size(cluster_all_index,1)
%                             vertex_values_sum(label == colortable.table(cluster_all_index(cluster_i),5),:) ...
%                                 = cluster_based_sig_roi(cluster_all_index(cluster_i),frame_index);
%                         end
                    % t value mapping
                    if zscore_flag == 1
                        for roi_i=1:size(cluster_all_index,1)
                            vertex_values_sum(label == colortable.table(cluster_all_index(roi_i),5),:) ...
                                = cluster_based_sig_roi(cluster_all_index(roi_i),frame_index);
                        end
                    else
                        for roi_i=1:size(cluster_all_index,1)
                            vertex_values_sum(label == colortable.table(cluster_all_index(roi_i),5),:) ...
...
%                 end
    ...
    cd(data_storage_folder_stats)
    if side_index == 1
        save(['L_vertex_values_' atlas_index '_' file_suffix '_' flip_prefix '_' num2str(r_time) '.mat'],'vertex_values_sum_all','-v7.3')
    else
        save(['R_vertex_values_' atlas_index '_' file_suffix '_' flip_prefix '_' num2str(r_time) '.mat'],'vertex_values_sum_all','-v7.3')
    end
end
...
%                 if laterality == 4 && ismember(overlay,[2 3 4])
%                     load(xtick_spacing_filename)
