...
function displayElectrodesInflated_new_fs_zscore(data_location,patients,overlay, laterality, views, thr,color_range,colormap_mat,file_suffix,storage_suffix,Montage_suffix,atlas_name,atlas_folder,atlas_index,flip_prefix)

% image_storage_folder = [data_location '/Storage_filtering_overlap_base/Image_storage/' file_suffix '/' Montage_suffix '/' atlas_index '_' atlas_name];
image_storage_folder = [data_location '/Storage_filtering_overlap_' storage_suffix '/Image_storage/' file_suffix '/' Montage_suffix '/' atlas_index '_zval'];
% data_storage_folder = [data_location '/Storage_filtering_overlap/Data_storage/' file_suffix '/' Montage_suffix ];
...
%         load('cmap_hunki_10to15.mat'); 
        ...
        if side_index == 1
            z_mean_all=nanmean(both_mean_vertex_values,3);
            zval_raw=z_mean_all(1:size(colortable.table,1),:);
        else
            z_mean_all=nanmean(both_mean_vertex_values,3);
            zval_raw=z_mean_all(size(colortable.table,1)+1:end,:);
        end
        ...
        for frame_index = 1:total_num_frames
            ...
            if (overlay == 2 || overlay == 3 || overlay == 4) || ~electrodeDensity% the surface is colored according to zscores of power
                    % t value mapping
                for roi_i=2:size(colortable.table,1)
                    vertex_values_sum(label == colortable.table(roi_i,5),:) ...
                        = zval_raw(roi_i,frame_index);
                end                       
